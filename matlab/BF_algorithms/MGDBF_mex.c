...
void MGDBF_full(double *H_full, double *input, double *output, double *counter_flip, int L, int w, int Z, int nb, int mb);
void MGDBF(int **H, int *x, double *y, int M, int N, double *counter_flip);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    plhs[1] = mxCreateDoubleMatrix(1, MAX_ITER, mxREAL); //mxCreateDoubleMatrix(1, MAX_ITER, mxREAL);
    double *num_flips = mxGetPr(plhs[1]);

    MGDBF_full(param_Hfull, param_input, param_output, num_flips, param_L, param_w, param_Z, param_nb, param_mb);
    return;
}
...
void MGDBF_full(double *H_full, double *input, double *output, double *counter_flip, int L, int w, int Z, int nb, int mb)
{
    ...
    for (int t = 0; t < L - 1; t++)
    {
        ...
        MGDBF(H, xhat, y, M, N, counter_flip);
...
{   
    ...
    for (int i_itera = 0; i_itera < MAX_ITER; i_itera++)
    {
        ...
        for (int k = 0; k < N; k++)
        {
            ...
            if (inverFunc < lowestValue || k == 0) //|| k == 0
            {
                lowestIdx = k;
        ...
        if(mode == 0)
        {
            ...
            for (int i = 0; i < flip_pos_length; i++)
            {
                x[flip_pos[i]] = -x[flip_pos[i]];
            }
