...
void MGDBFV2(int **H, int *x, double *y, int M, int N, double *counter_flip);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    plhs[1] = mxCreateDoubleMatrix(1, MAX_ITER, mxREAL); //mxCreateDoubleMatrix(1, 1, mxREAL);
...
void MGDBF_full(double *H_full, double *input, double *output, double *counter_flip, int L, int w, int Z, int nb, int mb)
{
    ...
    for (int t = 0; t < L - 1; t++)
    {
        ...
        int counter = 0;
        MGDBFV2(H, xhat, y, M, N, &counter_flip[t]);
